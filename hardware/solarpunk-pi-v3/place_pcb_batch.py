#!/usr/bin/env python3
"""Place footprints in batches to avoid memory issues on Pi 5.
Usage: QT_QPA_PLATFORM=offscreen python3 place_pcb_batch.py [batch_num]
  No args = run all batches sequentially.
  batch_num = run specific batch (0-5).
"""
...
# Batch definitions - each batch is a chunk of placements
# Format per line: ref,val,fpkey,x,y,side,rot
BATCHES = [
    # Batch 0: Top side - compute core + DDR + PMIC + decoupling
...
C24,10uF,C0805,17,36,F,0""",
    ...
    # Batch 1: Top side - connectivity (Ethernet, WiFi, SIM, Audio, USB, HDMI, CSI, GPIO, ESD)
...
U13,PRTR5V0U2X,SOT363,68,20,F,0""",
    ...
    # Batch 2: Bottom side - RP2350 radio module
...
Q2,Si2301,SOT23,26,24,B,0""",
    ...
    # Batch 3: Bottom side - RK3506J industrial + CAN + RS485
...
C63,100nF,C0402,64,50,B,0""",
    ...
    # Batch 4: Bottom side - power system
...
FB2,BLM18AG,R0603,40,40,B,0""",
    ...
    # Batch 5: Bottom side - signal conditioning
...
def parse_batch(batch_str):
    items = []
    for line in batch_str.strip().split('\n'):
        line = line.strip()
        if not line:
            continue
        parts = line.split(',')
        ref, val, fpk = parts[0], parts[1], parts[2]
        x, y = float(parts[3]), float(parts[4])
        side, rot = parts[5], float(parts[6])
        items.append((ref, val, fpk, x, y, side, rot))
    return items


def run_batch(batch_num, add_zones=False):
    """Run a single batch placement."""
    import pcbnew

    fpmap = parse_fpmap()
    items = parse_batch(BATCHES[batch_num])
    ...
    for ref, val, fpk, x, y, side, rot in items:
        if ref in existing:
            continue
    ...
    if add_zones:
        # GND zones on In1.Cu and In4.Cu
        ...
        for cx, cy in [(0,0),(85,0),(85,56),(0,56)]:
            o.Append(pcbnew.FromMM(cx), pcbnew.FromMM(cy))
        board.Add(z)
        print('  Added GND + 5V zone pours')

    board.Save(PCB)
    total = len(board.GetFootprints())
    print(f'  Batch {batch_num}: placed={placed} failed={failed} total_fps={total}')
...
def main():
    ...
    if args:
        # Run specific batch
        batch_num = int(args[0])
        add_zones = '--zones' in args
        run_batch(batch_num, add_zones)
    else:
        # Run all batches as separate processes to avoid memory issues
        ...
        for i in range(len(BATCHES)):
            ...
            if result.returncode != 0:
                print(f'  WARNING: Batch {i} exited with code {result.returncode}')

        # Final count
        import pcbnew
        board = pcbnew.LoadBoard(PCB)
        print(f'\nFinal: {len(board.GetFootprints())} footprints on board')
        print('DONE')
